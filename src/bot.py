...
from src.nlp import parse_message, parse_timeline_query
...
def _handle_record_event(message_id: str, open_id: str, text: str):
    ...
    if BASE_EVENT_TABLE:
        fields = {
            "日期时间": parsed.date_start,
            "事件内容": parsed.content,
            "原始消息": parsed.raw_content,
        }
        if parsed.date_end:
            fields["结束时间"] = parsed.date_end
...
def _find_or_create_project(name: str) -> str | None:
...
def _find_project_record_id(name: str) -> str | None:
    """查找项目记录 ID（精确匹配 + 别名匹配）"""
    if not BASE_PROJECT_TABLE:
        return None
